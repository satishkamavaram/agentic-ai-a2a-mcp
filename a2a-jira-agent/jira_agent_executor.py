...
# read it from request in real time
DEFAULT_USER_ID = '1234'


class JirarExecutor(AgentExecutor):
        ...
        try:
            async for event in self.runner.run_async(
                ...
                if not event.get_function_calls():
                    logger.debug('Yielding update response..found mcp server tools')
                    await task_updater.update_status(
                        TaskState.working,
    ...
    async def execute(
        ...
        logger.debug('[jira] execute exiting')

    async def cancel(self, context: RequestContext, event_queue: EventQueue):
       #TODO implement clear session for the requested session
